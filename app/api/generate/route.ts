...
export async function POST(req:Request){
 try{
  ...
  for(const candidate of order){ try { output=await generateWithProvider(candidate,model,buildPrompt({tool,platform,language,tone,topic,provider:candidate,model})); if(output){usedProvider=candidate;break;} lastProviderError=providerErrorMessage(new Error("empty_output"),candidate); } catch(error) { lastProviderError=providerErrorMessage(error,candidate); }
  }
