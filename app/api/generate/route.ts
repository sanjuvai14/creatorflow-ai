...
function configuredModel(provider: ConcreteProvider, requested?: string) { const item=getProviderConfigs().find((config)=>config.name===provider); return requested || item?.model; }
function providerErrorMessage(error: unknown, provider: ConcreteProvider) {
 ...
 if(provider === "openai") {
  ...
  if(status === 403 || message.includes("permission")) return "OpenAI denied access to the selected model. Check API project/model permissions.";
  if(status === 404 || message.includes("model")) return "The configured OpenAI model is unavailable for this API project. Check OPENAI_TEXT_MODEL.";
 }
...
async function generateWithProvider(provider: ConcreteProvider, model: string | undefined, prompt: string) {
 ...
 if(provider === "openai") { const apiKey=process.env.OPENAI_API_KEY; if(!apiKey) throw new Error("provider_not_configured"); const client=new OpenAI({apiKey}); const response=await withTimeout(()=>client.responses.create({model:selected()||"gpt-5-mini",input:prompt})); return response.output_text?.trim()||""; }
 if(provider === "gemini") { const apiKey=process.env.GEMINI_API_KEY; if(!apiKey) throw new Error("provider_not_configured"); const response=await withTimeout((signal)=>fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(selected()||"gemini-flash-latest")}:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":apiKey},body:JSON.stringify({contents:[{role:"user",parts:[{text:prompt}]}]}),signal})); const data=await extractText(response); return data?.candidates?.[0]?.content?.parts?.map((part:{text?:string})=>part.text||"").join("").trim()||""; }
 if(provider === "anthropic") { const apiKey=process.env.ANTHROPIC_API_KEY; if(!apiKey) throw new Error("provider_not_configured"); const response=await withTimeout((signal)=>fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":apiKey,"anthropic-version":"2023-06-01"},body:JSON.stringify({model:selected()||"claude-sonnet-4-5",max_tokens:4096,messages:[{role:"user",content:prompt}]}),signal})); const data=await extractText(response); return data?.content?.filter((part:{type?:string})=>part.type==="text").map((part:{text?:string})=>part.text||"").join("").trim()||""; }
 if(provider === "grok") { const apiKey=process.env.XAI_API_KEY; if(!apiKey) throw new Error("provider_not_configured"); const response=await withTimeout((signal)=>fetch("https://api.x.ai/v1/chat/completions",{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${apiKey}`},body:JSON.stringify({model:selected()||"grok-4.6",messages:[{role:"user",content:prompt}]}),signal})); const data=await extractText(response); return data?.choices?.[0]?.message?.content?.trim()||""; }
...
export async function POST(req:Request){
 try{
  ...
  let output=""; let usedProvider:ConcreteProvider|""=""; let lastProviderError="";
  const effectiveModel=requestedProvider==="auto"?undefined:model;
  for(const candidate of order){ try { output=await generateWithProvider(candidate,effectiveModel,buildPrompt({tool,platform,language,tone,topic,provider:candidate,model:effectiveModel})); if(output){usedProvider=candidate;break;} lastProviderError=providerErrorMessage(new Error("empty_output"),candidate); } catch(error) { lastProviderError=providerErrorMessage(error,candidate); }
  }
  if(!output){ const refund=await refundCredit(user.id); return NextResponse.json({error:refund.ok?lastProviderError||"AI generation failed. Please try again.":"AI generation failed and the credit could not be returned automatically. Please contact support.",credits:refund.credits??credits,code:"AI_GENERATION_FAILED",attemptedProviders:order},{status:502}); }
  ...
  if(!usedProvider){ const refund=await refundCredit(user.id); return NextResponse.json({error:refund.ok?"AI generation completed without a recognized provider.":"AI generation completed without a recognized provider and the credit could not be returned automatically.",credits:refund.credits??credits,code:"AI_PROVIDER_UNRESOLVED"},{status:502}); }
  return NextResponse.json({output,credits,provider:usedProvider,model:configuredModel(usedProvider,effectiveModel)||null,fallbackUsed:requestedProvider==="auto"&&usedProvider!==order[0]});
