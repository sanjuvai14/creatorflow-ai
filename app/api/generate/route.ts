...
export async function POST(req:Request){
 try{
  ...
  const tool=safeString(body?.tool); const platform=safeString(body?.platform); const language=safeString(body?.language); const tone=safeString(body?.tone); const topic=typeof body?.topic==="string"?body.topic.trim():"";
  ...
  try{
   ...
   const response=await client.responses.create({model:process.env.OPENAI_TEXT_MODEL||"gpt-5-mini",input:prompt}); output=response.output_text;
