...
import { getProviderConfigs, resolveProvider, providerIsConfigured, type AIProviderId, type AIProviderName } from "@/lib/ai/providers";
...
async function generateWithProvider(provider: ConcreteProvider, model: string | undefined, prompt: string) {
 ...
 if(provider === "openai") { if(!process.env.OPENAI_API_KEY) throw new Error("provider_not_configured"); const client=new OpenAI({apiKey:process.env.OPENAI_API_KEY}); const response=await withTimeout(()=>client.responses.create({model:selected()||"gpt-5-mini",input:prompt})); return response.output_text?.trim()||""; }
