...
async function generateWithProvider(provider: Exclude<AIProviderId,"auto">, model: string | undefined, prompt: string) {
 if(provider === "openai") {
  if(!process.env.OPENAI_API_KEY) throw new Error("provider_not_configured");
  const client=new OpenAI({apiKey:process.env.OPENAI_API_KEY});
  const response=await client.responses.create({model:model||process.env.OPENAI_TEXT_MODEL||"gpt-5-mini",input:prompt});
  return response.output_text?.trim()||"";
 }
 if(provider === "gemini") {
  if(!process.env.GEMINI_API_KEY) throw new Error("provider_not_configured");
  const selected=model||process.env.GEMINI_TEXT_MODEL||"gemini-3.6-flash";
  const response=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(selected)}:generateContent`,{method:"POST",headers:{"Content-Type":"application/json","x-goog-api-key":process.env.GEMINI_API_KEY},body:JSON.stringify({contents:[{role:"user",parts:[{text:prompt}]}]})});
...
export async function POST(req:Request){
 try{
  ...
  if(!provider){
   return NextResponse.json({error:requestedProvider==="auto"?"No AI provider is connected yet. Your credits were not used.":"That AI provider is not connected yet. Add its server-side API key before using it.",credits:null,code:"AI_PROVIDER_NOT_CONFIGURED"},{status:503});
  }
  if(!providerIsConfigured(provider)) return NextResponse.json({error:"Selected AI provider is not configured. Your credits were not used.",credits:null,code:"AI_PROVIDER_NOT_CONFIGURED"},{status:503});
  ...
  let output="";
  try{
   output=await generateWithProvider(provider,model,buildPrompt({tool,platform,language,tone,topic,provider,model}));
   if(!output)throw new Error("empty_output");
  }catch{
   const refund=await refundCredit(user.id);
   return NextResponse.json({error:refund.ok?"AI generation failed. Your credit has been returned; please try again.":"AI generation failed. We could not automatically return the credit. Please contact support.",credits:refund.credits??credits},{status:502});
  }
