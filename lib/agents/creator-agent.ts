...
export const CREATOR_AGENT_TOOLS: ToolDef[] = [
  { type: "function", name: "create_content_asset", description: "Create a copy-ready creator asset from a user goal. Use for YouTube, Instagram, TikTok, Facebook, LinkedIn, X, Pinterest, Shopify and general business content.", parameters: { type: "object", properties: { platform: { type: "string" }, asset_type: { type: "string" }, topic: { type: "string" }, language: { type: "string" }, tone: { type: "string" } }, required: ["platform", "asset_type", "topic", "language", "tone"], additionalProperties: false }, strict: true },
  { type: "function", name: "save_content", description: "Save user-approved generated content to the CreatorFlow library. Never use for secrets or sensitive personal data.", parameters: { type: "object", properties: { title: { type: "string" }, content: { type: "string" }, tool_type: { type: "string" } }, required: ["title", "content", "tool_type"], additionalProperties: false }, strict: true },
  { type: "function", name: "get_recent_content", description: "Read the user's recent saved generations to maintain continuity.", parameters: { type: "object", properties: { limit: { type: "integer", minimum: 1, maximum: 20 } }, required: ["limit"], additionalProperties: false }, strict: true },
  { type: "function", name: "build_content_plan", description: "Build a multi-platform content plan with topics, formats, hooks and scheduling suggestions. Does not publish anything.", parameters: { type: "object", properties: { goal: { type: "string" }, platforms: { type: "array", items: { type: "string" } }, days: { type: "integer", minimum: 1, maximum: 30 }, language: { type: "string" } }, required: ["goal", "platforms", "days", "language"], additionalProperties: false }, strict: true },
  { type: "function", name: "request_external_action_confirmation", description: "Prepare a confirmation request for an external or irreversible action such as publishing, sending, connecting an account, or spending money. This tool never performs the action.", parameters: { type: "object", properties: { action: { type: "string" }, details: { type: "string" } }, required: ["action", "details"], additionalProperties: false }, strict: true }
];
...
export async function runCreatorAgent(input: string, ctx: AgentToolContext) {
  ...
  const instructions = `You are CreatorFlow AI Agent, an autonomous creator and business workspace agent. You can reason across multi-step tasks, use approved tools, remember recent CreatorFlow content, and prepare creator workflows. Prefer the simplest reliable tool sequence. Never claim an action happened unless a tool actually completed it. Never fabricate analytics or live metrics. Never promise guaranteed followers, virality, watch time, sales or income. External actions (publishing, sending messages, connecting accounts, financial/payment actions, or other irreversible actions) require explicit user confirmation immediately before execution. The current toolset intentionally does not execute those actions. Use the user's language when practical. Return concise but useful results. For content requests, produce copy-ready output. For multi-step requests, complete as much as possible autonomously, then report what was completed and what requires confirmation.`;
  const tools = [...CREATOR_AGENT_TOOLS, { type: "web_search" }] as any;

  // Keep response state available for tool continuation. Using previous_response_id
  // requires stored Responses API state, so do not disable storage here.
...
export async function runAgentForUser(input: string, userId: string) {
  ...
  try { const result = await runCreatorAgent(input, { userId }); return { ...result, credits }; }
