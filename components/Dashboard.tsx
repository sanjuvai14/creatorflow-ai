...
export default function Dashboard(){
 ...
 <div className="cf-workspace"><aside className="cf-sidebar cf-card"><div className="cf-sidebar-title">TOOLS</div>{tools.map(t=><button key={t.id} className={"cf-tool "+(tool===t.id&&!platform?"active":"")} onClick={()=>chooseTool(t.id)}><span className="cf-tool-icon">{t.icon}</span><span><b>{t.name}</b><small>{t.desc}</small></span><i>›</i></button>)}<button className="cf-tool" onClick={goPlatforms}><span className="cf-tool-icon">▦</span><span><b>Platform Hub</b><small>All platforms, workflows & integrations</small></span><i>›</i></button><div className="cf-sidebar-foot"><span>✦</span><div><b>CreatorFlow AI</b><small>Your all-in-one creator workspace</small></div></div></aside>
 <section className="cf-main">{calendarTool(tool)&&platform?<ContentCalendar/>:growthTool(tool)&&platform?<GrowthTools tool={tool} platform={platform}/>:tool==="visual"&&!platform?<VisualCreator/>:<div className="cf-card cf-generator"><div className="cf-section-head"><div><div className="cf-eyebrow">AI CHAT WORKSPACE</div><h2>What do you want to create?</h2><p>Ask in your own words. CreatorFlow will turn your idea into a ready-to-use draft.</p></div><span className="cf-live">● Ready</span></div>
 <div style={{display:"flex",gap:8,flexWrap:"wrap",margin:"0 0 18px"}}>{starterIdeas.map(idea=><button key={idea} className="cf-icon-btn" style={{fontSize:12}} onClick={()=>useIdea(idea)}>{idea}</button>)}</div>
 {messages.length>0&&<div style={{display:"grid",gap:12,marginBottom:18,maxHeight:430,overflowY:"auto"}}>{messages.map((m,i)=><div key={i} style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}><div style={{maxWidth:"88%",padding:"13px 15px",borderRadius:16,background:m.role==="user"?"rgba(139,124,255,.18)":"rgba(255,255,255,.045)",border:"1px solid rgba(255,255,255,.08)",whiteSpace:"pre-wrap",lineHeight:1.6}}><div style={{fontSize:11,opacity:.6,marginBottom:5}}>{m.role==="user"?"You":"CreatorFlow AI"}</div>{m.text}</div></div>)}</div>}
 <div className="cf-chat-input" style={{display:"grid",gap:10}}><textarea className="cf-input cf-topic" style={{minHeight:125}} placeholder="Example: Write a 60-second YouTube script about how beginners can use AI to save time…" value={topic} onChange={e=>setTopic(e.target.value)} onKeyDown={e=>{if((e.ctrlKey||e.metaKey)&&e.key==="Enter")generate()}}/><div style={{display:"flex",gap:8,justifyContent:"space-between",alignItems:"center",flexWrap:"wrap"}}><span className="cf-muted" style={{fontSize:12}}>Ctrl/⌘ + Enter to generate • {activeTool.name}</span><button className="cf-btn cf-generate" onClick={generate} disabled={loading||!topic.trim()}>{loading?"Thinking…":"✦ Ask CreatorFlow"}</button></div></div>
 <details style={{marginTop:16}}><summary style={{cursor:"pointer",fontSize:13,opacity:.8}}>More controls (optional)</summary><div className="cf-form-grid" style={{marginTop:14}}><label>Language<select className="cf-input" value={language} onChange={e=>setLanguage(e.target.value)}>{languages.map(x=><option key={x}>{x}</option>)}</select></label><label>Tone<select className="cf-input" value={tone} onChange={e=>setTone(e.target.value)}>{tones.map(x=><option key={x}>{x}</option>)}</label></div></details>
 {error&&<div role="alert" style={{marginTop:14,padding:"12px 14px",borderRadius:10,border:"1px solid rgba(255,120,120,.35)"}}>{error}</div>}
 {result&&<div className="cf-result" style={{marginTop:18}}><div className="cf-result-head"><div><div className="cf-eyebrow">LATEST OUTPUT</div><h2>Ready to use</h2></div><div className="cf-result-actions"><span>{action}</span><button className="cf-icon-btn" onClick={copyResult}>Copy</button><button className="cf-icon-btn" onClick={saveResult} disabled={action==="Saving..."}>Save</button></div></div><pre>{result}</pre></div>}
