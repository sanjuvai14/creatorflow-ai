...
export default function Dashboard() {
  ...
  async function copyResult(){
    if(!result) return;
    try { await navigator.clipboard?.writeText(result); setAction("Copied!"); setTimeout(()=>setAction(""),1600); }
    catch { setAction("Copy failed"); setTimeout(()=>setAction(""),1600); }
  }
  ...
  async function saveResult(){
    ...
    setAction("Saving...");
    try {
      const r=await fetch("/api/save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:topic.slice(0,60),type:tool,content:result})});
      const data=await r.json();
  ...
  return <main style={{maxWidth:1280, margin:"0 auto", padding:"20px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"280px 1fr",gap:20}}>
      <aside className="cf-card" style={{padding:16,height:"fit-content"}}>
        ...
        {tools.map(t=><button key={t.id} onClick={()=>{setTool(t.id);setAction("")}} style={{display:"block",width:"100%",textAlign:"left",padding:13,marginBottom:8,borderRadius:12,border:"1px solid "+(tool===t.id?"rgba(124,92,255,.7)":"transparent"),background:tool===t.id?"rgba(124,92,255,.16)":"transparent",color:"white",cursor:"pointer"}}>
        ...
        {tool==="visual" ? <VisualCreator/> : <>
          ...
          <div className="cf-card" style={{padding:24,marginTop:20,minHeight:260}}>
            <div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}><span className="cf-muted" style={{fontSize:13,minHeight:18}}>{action}</span><button className="cf-btn" style={{background:"rgba(255,255,255,.08)"}} onClick={copyResult} disabled={!result}>Copy</button><button className="cf-btn" style={{background:"rgba(255,255,255,.08)"}} onClick={saveResult} disabled={!result||action==="Saving..."}>Save</button></div>
            </div>
