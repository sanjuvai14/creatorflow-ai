...
export default function AuthForm() {
  ...
  async function submit(e:React.FormEvent){
    ...
    try {
      ...
      if(mode==="signup"&&!result.data.session){setMsg("Account created. Check your email to confirm your account, then log in.");return;}
      router.replace(routeNext); router.refresh();
    } catch { setMsg("Something went wrong. Please try again."); }
    finally { setLoading(false); }
  }

  async function google(){
    setMsg(""); const supabase=createClient();
  ...
  return <div className="cf-card" style={{padding:26}}>
    ...
    <form onSubmit={submit} style={{display:"grid",gap:12}}>
      ...
      <input className="cf-input" placeholder="Password" type="password" value={password} onChange={e=>setPassword(e.target.value)} required minLength={6} />
    ...
    <p className="cf-muted" style={{fontSize:12,marginTop:18}}>Create an account first if you are new. Email confirmation may be required by your Supabase Auth settings.</p>
  </div>;
}
