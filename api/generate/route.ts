  ...
  try {
    ...
    try {
      ...
      if (!output) {
        const refund = await refundCredit(user.id);
        return NextResponse.json({
          error: refund.ok ? "AI returned an empty result. Your credit has been returned; please try again." : "AI returned an empty result, and the credit could not be returned automatically. Please contact support.",
          credits: refund.credits ?? credits,
        }, { status: 502 });
      }
    } catch {
